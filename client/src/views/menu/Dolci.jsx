...
const Dolci = () => {
    ...
    const fetchData = async () => {
        ...
        const cachedData = localStorage.getItem('dolciItems')
        const cachedTime = localStorage.getItem('dolciItemsTime')
        ...
        if (cachedData && cachedTime && (now - cachedTime < CACHE_DURATION)) {
            ...
            const res = await findByCategory('Dolci')
            setProdotti(res)
            localStorage.setItem('dolciItems', JSON.stringify(res))
            localStorage.setItem('dolciItemsTime', now)
            setLoading(false)
        }
    }
